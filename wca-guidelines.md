...
- 1h++) [RECOMMENDATION] If conditions permit all attempts in the final round of an event, as well as all Fewest Moves Solving attempts, should have the same scramble sequences for all competitors (i.e. only 1 group).
- 1h+++) [CLARIFICATION] Each subset of competitors competing in a round is considered a group.
...
- 3d1b+) [CLARIFICATION] Patterns do not need to be present on every face, but only on faces where a distinct colour cannot be found. Pieces must not have any feature (e.g. textures, pattern irregularities) that significantly distinguishes them from similar pieces.
...
- A7g+) [EXAMPLE] Examples of incidents that could award a new competitor an extra attempt: not waiting for the green light, improperly starting or stopping the timer, going over inspection time due to misunderstanding how to start an attempt, or other procedural time penalties.
- A7g++) [CLARIFICATION] Individual penalties cannot be waived for new competitors. A full extra attempt must be given.
- A7g+++) [RECOMMENDATION] The WCA Delegate should use stronger discretion when awarding more than one extra attempt to the same new competitor. If more than one extra attempt is awarded, it should be for a different incident.
